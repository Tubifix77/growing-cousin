...
| `0914-healthy-hour` | 09‑14 22:00 → 23:00 | cut as the control. On 09‑15 it turned out to carry the chooser fault — `view-subtask-logs` probed 5 of 5, bare — so it is a control for every detector **except `probe_stuck`**, which must fire on it | all but one quiet; `probe_stuck` |
