...
| 8b | **An `ask` hand ALIGNED with the spine's -- DECIDED by Tue 2026-09-24**: *"if we are ever going to compare the two we need them to be aligned ... even if it means it is part framework"*. Identical from inside the box: same name and call line, same model (`openai/gpt-oss-120b` on Groq), same contract (stdout is the answer or EMPTY, every failure on stderr with a nonzero exit), same readable budget file (`/mind/state/ask_quota.json`), same 500/day cap. **The one invisible difference: the key never enters the box** -- `ask` hands the prompt to a small relay on the laptop that holds the key, counts the budget and forwards it. NOT via the cousin: the spine's `ask` is instant and plain, the cousin only comes on triggers, answers as a judge, and is forbidden to do the creature's work | Claude | after step 4's read, and 8a | the creature's `ask` behaves byte-for-byte like the spine's on the same inputs (rehearsed on the bench), `selfcheck` still proves `keys_unreadable`, and the relay refuses past its cap |
| 8c | **The one gap left, documented not closed**: the spine's creature ALSO holds every enabled rung's raw key in its environment (`sandbox.container_api_env`), and 4 of its 929 tools call an API directly; 124 go through `ask`. Ours gets `ask` only. Closing it would mean keys in the box | -- | -- | recorded in `CLAUDE.md` §4 |
